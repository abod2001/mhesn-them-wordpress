 ...
 * @since      1.0.0
 */
class mhesn_Recent_Widget extends WP_Widget {
	...
	function __construct() {
		...
		$widget_options = array(
			'classname'   => 'widget-mhesn-recent widget-posts-thumbnail',
			'description' => __( 'Display recent posts with thumbnails.', 'mhesn' )
		);

		// Create the widget.
		parent::__construct(
			'mhesn-recent',                                   // $this->id_base
			__( '&raquo; Recent Posts', 'mhesn' ), // $this->name
	...
	function widget( $args, $instance ) {
		...
		if ( false === ( $recent = get_transient( 'mhesn_recent_widget_' . $this->id ) ) ) {
			...
			// Store the transient.
			set_transient( 'mhesn_recent_widget_' . $this->id, $recent );
	...
	function flush_widget_transient() {
		delete_transient( 'mhesn_recent_widget_' . $this->id );
	}
	...
	function form( $instance ) {
		...
		// Default value.
		$defaults = array(
			'title'     => esc_html__( 'Recent Posts', 'mhesn' ),
			'limit'     => 5,
			'show_date' => true
		);
