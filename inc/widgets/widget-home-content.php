 ...
 * @since      1.0.0
 */
class mhesn_Home_Content_Widget extends WP_Widget {
	...
	function __construct() {
		...
		$widget_options = array(
			'classname'   => 'widget-mhesn-home-block-one',
			'description' => __( 'Display post content blocks. Only use for the "Home Content" widget area.', 'mhesn' )
		);

		// Create the widget.
		parent::__construct(
			'mhesn-home-block-one',          // $this->id_base
			__( '&raquo; Home Content', 'mhesn' ), // $this->name
	...
	function widget( $args, $instance ) {
		...
		echo wp_kses_post( $before_widget );
			...
			if ($col == '4col') {
				$grid_col = 'ht_grid_1_4 ht_grid_m_1_2';
			} else {
				$grid_col = 'ht_grid_1_3 ht_grid_m_1_2';
			}
	...
	?>
		...
		<p>
			<label for="<?php echo esc_html( $this->get_field_id('col') ); ?>">
				...
				<?php esc_html_e( '3 Columns Layout', 'mhesn' ); ?>		    	
			</label>		    
		<br/>
			<label for="<?php echo esc_html( $this->get_field_id('col') ); ?>">
		    	...
		    	<?php esc_html_e( '4 Columns Layout', 'mhesn' ); ?>
			</label>		    
	    </p>
